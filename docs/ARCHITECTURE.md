...
- `LedService`: layered NEW/LEGACY LED engine selection, ambient, dimming, preview, and boot show.
...
- Available skins are `Coronet` (the original simple cyan/amber dashboard direction), `Graphite`, `Aurora`, and `Minimal`.
...
- Chamber telemetry is normalized through one time-aware EMA before entering `SystemState`. Its six-second time constant reproduces the coroNET 1 filter at the old polling cadence while remaining stable with faster WebSocket updates and slower HTTP fallback samples.
