...
// This is the exact gamma-2 output curve used by coroNET 1. Keep the inverse
// preview table paired with it so the LCD represents the physical PWM frame.
constexpr uint8_t InverseGamma20[256] = {
...
uint8_t encode(uint8_t perceived) {
    return static_cast<uint8_t>(
        (static_cast<uint16_t>(perceived) * perceived) >> 8U);
}

uint8_t decode(uint8_t pwm) {
    return InverseGamma20[pwm];
}
