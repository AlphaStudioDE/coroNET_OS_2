...
Place WAV files either in the card root or in `/sounds`. Subfolders inside `/sounds` are supported up to three levels deep. The selectable library is bounded to 64 files and sorted alphabetically, keeping startup time and PSRAM use predictable.
...
4. Select a WAV file from the paged list, or choose **Default**.
