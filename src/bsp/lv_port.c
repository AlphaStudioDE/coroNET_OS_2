...
lv_disp_t *lvgl_port_add_disp(const lvgl_port_display_cfg_t *disp_cfg)
{
    ...
    /* VERSION MARKER — always printed regardless of log level */
    esp_rom_printf("[LV_PORT] v3: buf=%lu px hres=%lu trans_from_bsp=%lu psram=%d\n",
                   (unsigned long)disp_cfg->buffer_size,
                   (unsigned long)disp_cfg->hres,
    ...
    /* -- PSRAM canvas mode --------------------------------------------------
     * When PSRAM is present and the draw buffer is large we ALWAYS:
     *   1. Allocate buf1 (draw buffer) in PSRAM, not DMA.
     *   2. Override trans_size to hres * 16 lines regardless of what the BSP
     *      passed in. The BSP default (hres*vres/10) wastes about 92 KB of
     *      DMA across two transfer buffers for no visual benefit here.
     *      hres * 16 = 480*16*2 = 15 360 B per buffer, 30 720 B total DMA.
     *
     * NOTE: disp_ctx->trans_size is set HERE, not copied from disp_cfg,
     * so the BSP value is intentionally ignored when PSRAM is available.
     * --------------------------------------------------------------------- */
    const uint32_t draw_bytes = disp_cfg->buffer_size * sizeof(lv_color_t);

    if (draw_bytes >= 32768 && esp_psram_is_initialized()) {
        buf1 = heap_caps_malloc(draw_bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (buf1) {
            /* Force a small transfer window and ignore the BSP trans_size value. */
            disp_ctx->trans_size = disp_cfg->hres * 16;
            esp_rom_printf("[LV_PORT] PSRAM canvas OK: draw=%luB PSRAM, trans=%lu px (%luB DMA)\n",
                     (unsigned long)draw_bytes,
                     (unsigned long)disp_ctx->trans_size,
                     (unsigned long)(disp_ctx->trans_size * sizeof(lv_color_t)));
        } else {
            /* PSRAM alloc failed — fall back to BSP trans_size and DMA */
            disp_ctx->trans_size = disp_cfg->trans_size;
            esp_rom_printf("[LV_PORT] PSRAM FAILED (%luB) — DMA fallback, trans=%lu\n",
                     (unsigned long)draw_bytes, (unsigned long)disp_ctx->trans_size);
        }
    } else {
        /* No PSRAM or small buffer — use BSP value as-is */
        disp_ctx->trans_size = disp_cfg->trans_size;
        esp_rom_printf("[LV_PORT] DMA path: trans=%lu\n", (unsigned long)disp_ctx->trans_size);
    }

    /* Fallback / explicit DMA path */
    if (!buf1) {
        uint32_t buff_caps = MALLOC_CAP_DMA;
        if (disp_cfg->flags.buff_spiram) {
            buff_caps = MALLOC_CAP_SPIRAM;
        } else if (!disp_cfg->flags.buff_dma) {
            buff_caps = MALLOC_CAP_DEFAULT;
        }
        buf1 = heap_caps_malloc(draw_bytes, buff_caps);
